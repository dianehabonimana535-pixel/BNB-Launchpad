...
import { Copy, ExternalLink, Trash2, Coins, BarChart3, X as XIcon, Droplets } from "lucide-react";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import Footer from "@/components/Footer";
import { getHistory, clearHistory, TokenHistoryEntry } from "@/lib/history";
import { NETWORKS, explorerAddressUrl, explorerTxUrl, buildShareOnXUrl, DEFAULT_NETWORK } from "@/lib/network";
      ...
      <div className="container py-12">
        ...
        {entries.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {entries.map((entry) => {
              const network = NETWORKS[entry.chain || DEFAULT_NETWORK];
              return (
                <Card key={entry.txHash}>
                  <CardContent className="space-y-3 pt-6">
                    ...
                    <button
                      onClick={() => copy(entry.tokenAddress)}
                      className="flex items-center gap-1.5 font-mono text-xs text-muted-foreground hover:text-foreground"
                    >
                      {shortenAddress(entry.tokenAddress, 6)} <Copy className="h-3 w-3" />
                    </button>
                    <div className="flex gap-2 pt-1">
                      <Button asChild variant="outline" size="sm" className="flex-1">
                        <a href={explorerAddressUrl(network, entry.tokenAddress)} target="_blank" rel="noreferrer">
                          <ExternalLink className="h-3.5 w-3.5" /> Token
                        </a>
                      </Button>
                      <Button asChild variant="outline" size="sm" className="flex-1">
                        <a href={explorerTxUrl(network, entry.txHash)} target="_blank" rel="noreferrer">
                          <ExternalLink className="h-3.5 w-3.5" /> Tx
                        </a>
                      </Button>
                    </div>
                    <Button asChild variant="gradient" size="sm" className="w-full">
                      <Link href={`/token/${entry.tokenAddress}?chain=${network.key}`}>
                        <BarChart3 className="h-3.5 w-3.5" /> Dashboard
                      </Link>
                    </Button>
                    <Button asChild variant="outline" size="sm" className="w-full">
                      <a
                        href={network.addLiquidityUrl(entry.tokenAddress)}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <Droplets className="h-3.5 w-3.5" /> Add Liquidity on {network.dexName}
                      </a>
                    </Button>
                    <Button asChild variant="outline" size="sm" className="w-full">
                      <a
                        href={buildShareOnXUrl(network, entry.name, entry.symbol, entry.tokenAddress, entry.revokedCount)}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
