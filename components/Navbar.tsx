...
import { NETWORKS, DEFAULT_NETWORK } from "@/lib/network";
...
export default function Navbar() {
  ...
  const [liquidityHref, setLiquidityHref] = useState("https://pancakeswap.finance/liquidity");

  useEffect(() => {
    const stored = window.localStorage.getItem("bnbmint.theme");
  ...
  useEffect(() => {
    ...
    if (history.length > 0) {
      ...
      const network = NETWORKS[lastToken.chain || DEFAULT_NETWORK];
      setLiquidityHref(network.addLiquidityUrl(lastToken.tokenAddress));
    }
  }, [pathname]);
  ...
  function toggleTheme() {
    ...
    window.localStorage.setItem("bnbmint.theme", next ? "light" : "dark");
  }
