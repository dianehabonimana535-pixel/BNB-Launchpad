...
import { explorerTokenUrl, explorerTxUrl, buildShareOnXUrl } from "@/lib/network";
...
// setTimeout: each transition is driven by a genuine event from the
// deployment flow. See handleSubmit's onStep handler below for the exact
// mapping.
const FLOW_MESSAGES = [
...
export default function TokenCreatorForm() {
  ...
  const [resultNetwork, setResultNetwork] = useState(wallet.targetNetwork);

  // Whether the custom progress popup is currently visible. This only ever
  // becomes true once lib/mint.ts reports the "signed" step, i.e. once the
  // connected wallet (MetaMask, Trust Wallet, etc.) has actually returned
  // a signed, broadcast transaction. It is never shown before that, and
  // never shown at all if the user rejects the wallet prompt.
  ...
  const totalFeeNative = estimateFees(wallet.targetNetwork.key, revokeCount).totalNative;
  const nativeSymbol = wallet.targetNetwork.nativeSymbol;
  ...
  function onMintStep(step: MintStep) {
    ...
    // returned anything - the custom popup stays hidden during these. The
    // button shows a spinner + "Waiting for wallet approval..." and the
    // wallet's own extension shows its native approval popup here.
    if (step === "signed") {
      // The connected wallet has just returned a signed, broadcast
      // transaction. This is the exact, real moment to reveal our popup.
      flowStepRef.current = 0;
      setFlowStep(0); // "Confirming transaction"
      setShowModal(true);
    } else if (step === "sent") {
      animateFlowTo(2); // reveals "Transaction received" then "Deploying token contract"
    } else if (step === "confirmed") {
      // The transaction is mined and confirmed on-chain: the contract now
      // exists and the full supply has been sent to the recipient wallet,
      // atomically, in the same deployment transaction. Reveal the
      // remaining messages in sequence, then hand off to the success
      // screen once "Complete" has been shown.
      animateFlowTo(5, () => {
  ...
  async function handleSubmit() {
    ...
    if (!wallet.connected || !wallet.address || !wallet.signer || !wallet.provider) {
      toast.error("Connect your wallet first");
      return;
    }
    if (wallet.isWrongNetwork) {
      toast.error(`Switch your wallet to ${wallet.targetNetwork.label} first`);
      return;
    }
    ...
    try {
      ...
      const recipient = form.recipient.trim() || wallet.address;

      const deployResult = await createToken({
      ...
      setResultNetwork(wallet.targetNetwork);
      ...
      addHistoryEntry({
        ...
        chain: wallet.targetNetwork.key,
      });
      ...
      const rejected =
        ...
        /reject|declin|cancel|user denied/i.test(err.message);
          ...
          <CardContent className="flex flex-col items-center gap-4 pt-8 text-center">
            ...
            <div className="w-full space-y-1.5 rounded-xl border border-border bg-secondary/30 p-4 text-left">
              ...
              <div className="flex items-center justify-between gap-2 font-mono text-sm text-accent">
                <span className="truncate">{result.tokenAddress}</span>
              </div>
            </div>
            ...
            <Button
              variant="gradient"
              className="w-full"
              onClick={() => copy(result.tokenAddress, "Token address")}
            >
            ...
            <div className="flex w-full flex-col gap-2 sm:flex-row">
              <Button asChild variant="outline" className="flex-1">
                <a href={explorerTokenUrl(resultNetwork, result.tokenAddress)} target="_blank" rel="noreferrer">
                  <ExternalLink className="h-4 w-4" /> View Token on {resultNetwork.explorerName}
                </a>
              </Button>
              <Button asChild variant="outline" className="flex-1">
                <a href={explorerTxUrl(resultNetwork, result.txHash)} target="_blank" rel="noreferrer">
            ...
            <Button asChild variant="outline" className="w-full">
              <a href={resultNetwork.addLiquidityUrl(result.tokenAddress)} target="_blank" rel="noreferrer">
                <Droplets className="h-4 w-4" /> Create Liquidity Pool on {resultNetwork.dexName}
              </a>
            </Button>

            <Button asChild variant="outline" className="w-full">
              <a
                href={buildShareOnXUrl(resultNetwork, form.name, form.symbol, result.tokenAddress, revokeCount)}
                target="_blank"
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              {wallet.connected && wallet.isWrongNetwork && (
                <p className="mt-2 text-xs text-amber-400">
                  Your wallet is connected to a different network. It will ask you to switch to{" "}
                  {wallet.targetNetwork.label} when you create the token.
                </p>
              )}
            </CardHeader>
            <CardContent className="space-y-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Decimals" required hint="18 is the standard for both BNB Chain (BEP-20) and Robinhood Chain (ERC-20)">
                  <Input type="number" min={0} max={18} value={form.decimals} onChange={(e) => update("decimals", e.target.value)} />
                </Field>
              ...
              <Field label="Recipient wallet" hint="Defaults to your connected wallet if left blank">
                <Input
                  placeholder={wallet.address || "Connect wallet for default"}
          ...
          <div className="flex items-center justify-between rounded-xl border border-border bg-secondary/30 px-4 py-3">
            ...
            <span className="font-mono text-base font-semibold gradient-text">{totalFeeNative.toFixed(6)} {nativeSymbol}</span>
          </div>

          <p className="text-xs text-muted-foreground">
            {"\u26A0\uFE0F"} Your wallet will be charged the gas amount shown above (
            <span className="font-mono font-semibold text-red-500">~{totalFeeNative.toFixed(6)} {nativeSymbol}</span>
            ), based on typical {wallet.targetNetwork.label} gas prices. Your wallet's confirmation popup shows
            the exact, live gas price and final total before you sign.
          </p>
          ...
          >
            ...
            {!wallet.connected
              ...
              ? `Switch to ${wallet.targetNetwork.label} to continue`
              : phase !== "running"
              ? `Create token on ${wallet.targetNetwork.label}`
              : showModal
              ? "Creating token..."
...
function validate(
  ...
  if (form.recipient.trim() && !isAddress(form.recipient.trim())) {
    errors.push("Recipient wallet address is not a valid EVM (0x...) address");
  }
  ...
  if (customCreatorEnabled) {
    ...
    } else if (!isAddress(form.creatorAddress.trim())) {
      errors.push("Creator's address is not a valid EVM (0x...) address");
    }
